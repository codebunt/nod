...
public class FacebookProvider extends AbstractLoginProvider implements LoginProvider {
	...
	public long signup(Map<String, String[]> data) throws InvalidDataException {
		...
		long fbid = getFacebookIdByCode(fbcode );
		Facebook fbObj = getByFBId(fbid);
		if(fbObj != null && fbObj.getUserId() != null) throw new InvalidDataException("Already Signed Up");
		if(fbObj == null)
			fbObj = new Facebook();
	...
	private long getFacebookIdByCode(String fbcode) throws InvalidDataException {
		...
		if(token == null) {
			throw new InvalidDataException("Invalid Code "+fbcode);
		}
		String url = "https://graph.facebook.com/me?fields=id&access_token="+URLEncoder.encode(token)+"&access_token="+token;
		try {
			boolean hasId = Unirest.get(url).asJson().getBody().getObject().has("id");
			if(!hasId) {
				throw new InvalidDataException("Invalid Access Key "+token);
			}
			return Unirest.get(url).asJson().getBody().getObject().getLong("id");
		} catch (UnirestException e) {
			throw new InvalidDataException(e);
		}
	}
	...
	public long link(String userid, Map<String, String[]> data) throws InvalidDataException {
		...
		String fbcode = getValue(data,"code");
		if(fbcode == null) {
			throw new InvalidDataException("Null fbcode");
		}
		long fbid = getFacebookIdByCode(fbcode);
		Facebook fbObj = getByFBId(fbid);
		if(fbObj == null) 
	...
	public long unlink(String userid, Map<String, String[]> data) throws InvalidDataException {
		...
		long fbid = getFacebookIdByCode(fbcode);
		Facebook fbObj = getByFBId(fbid);
		if(fbObj == null) throw new InvalidDataException("FB Account wasn't linked");
	...
	public AuthData signin(Map<String, String[]> data) throws InvalidDataException {
		...
		long fbid = getFacebookIdByCode(fbcode);
		Facebook fbObj = getByFBId(fbid);
		if(fbObj == null) throw new InvalidDataException("Facebook Account not signed up");
